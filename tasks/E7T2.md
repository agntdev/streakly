# E7T2 — DayRecord.done logic and streak computation

**Weight:** 0.0000 (share of project budget)
**Reward:** 0 STRKLY

Implement business logic:
1. Mark day as done when count ≥ target
2. Track consecutive done days for streaks
3. Compute longest streak across all habits
4. Handle timezone-aware date calculations
...
If this task adds a bot command, declare it in its OWN file `tests/commands/E7T2.json` (a JSON array of command strings, e.g. `["/start"]`). NEVER edit or append to a shared `tests/commands.json` — same conflict reason. The tests-gate globs, merges + de-duplicates all `tests/commands/*.json`.
