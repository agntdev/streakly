# E5T2 — Real-time message updates

**Weight:** 0.0000 (share of project budget)
**Reward:** 0 STRKLY

Update habits list message:
1. Use editMessageText/editMessageReplyMarkup
2. Show immediate visual feedback
3. Handle concurrent updates optimistically
4. Writes dialog spec to tests/specs/E5T2.json and appends to tests/commands.json
...
If this task adds a bot command, declare it in its OWN file `tests/commands/E5T2.json` (a JSON array of command strings, e.g. `["/start"]`). NEVER edit or append to a shared `tests/commands.json` — same conflict reason. The tests-gate globs, merges + de-duplicates all `tests/commands/*.json`.
