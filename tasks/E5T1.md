# E5T1 — /check command via inline buttons

**Weight:** 0.0000 (share of project budget)
**Reward:** 0 STRKLY

Implement habit check logic:
1. '✓ Done' marks daily/weekly as done
2. '+1' increments times/day count
3. 'Undo' decrements count
4. Writes dialog spec to tests/specs/E5T1.json and appends to tests/commands.json
...
If this task adds a bot command, declare it in its OWN file `tests/commands/E5T1.json` (a JSON array of command strings, e.g. `["/start"]`). NEVER edit or append to a shared `tests/commands.json` — same conflict reason. The tests-gate globs, merges + de-duplicates all `tests/commands/*.json`.
