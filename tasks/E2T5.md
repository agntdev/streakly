# E2T5 — Habit creation confirmation

**Weight:** 0.0000 (share of project budget)
**Reward:** 0 STRKLY

Finalize habit creation:
1. Confirm habit details
2. Return to main habits list message
3. Update message with new habit
4. Writes dialog spec to tests/specs/E2T5.json and appends to tests/commands.json
...
If this task adds or changes user-facing bot behavior, author its dialog tests as a `BotSpec` JSON array in its OWN file `tests/specs/E2T5.json`. NEVER edit or append to a shared `tests/specs.json` — concurrent feature PRs would conflict on it. The tests-gate globs and merges all `tests/specs/*.json`.

If this task adds a bot command, declare it in its OWN file `tests/commands/E2T5.json` (a JSON array of command strings, e.g. `["/start"]`). NEVER edit or append to a shared `tests/commands.json` — same conflict reason. The tests-gate globs, merges + de-duplicates all `tests/commands/*.json`.
