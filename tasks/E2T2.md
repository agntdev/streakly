# E2T2 — Frequency selection buttons

**Weight:** 0.0000 (share of project budget)
**Reward:** 0 STRKLY

Add inline buttons for habit frequency type:
1. Daily, X times/day, Weekly
2. Branch to appropriate setup (E2T3/E2T4)
3. Writes dialog spec to tests/specs/E2T2.json and appends to tests/commands.json
...
If this task adds a bot command, declare it in its OWN file `tests/commands/E2T2.json` (a JSON array of command strings, e.g. `["/start"]`). NEVER edit or append to a shared `tests/commands.json` — same conflict reason. The tests-gate globs, merges + de-duplicates all `tests/commands/*.json`.
