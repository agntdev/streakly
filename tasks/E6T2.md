# E6T2 — Back button to return to list

**Weight:** 0.0000 (share of project budget)
**Reward:** 0 STRKLY

Add navigation back button:
1. Show 'Back' in stats overlay
2. Return to main habits list
3. Preserve message state
4. Writes dialog spec to tests/specs/E6T2.json and appends to tests/commands.json
...
If this task adds a bot command, declare it in its OWN file `tests/commands/E6T2.json` (a JSON array of command strings, e.g. `["/start"]`). NEVER edit or append to a shared `tests/commands.json` — same conflict reason. The tests-gate globs, merges + de-duplicates all `tests/commands/*.json`.
