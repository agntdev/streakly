# E3T2 — Habit progress display

**Weight:** 0.0000 (share of project budget)
**Reward:** 0 STRKLY

Show habit progress in list:
1. 'Today: 0/3' for times/day habits
2. 'Done ✅' for completed habits
3. '🔥 4d' streak indicator
4. Writes dialog spec to tests/specs/E3T2.json and appends to tests/commands.json
...
If this task adds a bot command, declare it in its OWN file `tests/commands/E3T2.json` (a JSON array of command strings, e.g. `["/start"]`). NEVER edit or append to a shared `tests/commands.json` — same conflict reason. The tests-gate globs, merges + de-duplicates all `tests/commands/*.json`.
