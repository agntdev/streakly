# E2T3 — Times-per-day target setup

**Weight:** 0.0000 (share of project budget)
**Reward:** 0 STRKLY

Implement numeric target selection:
1. Show 1-6 number chooser with +/− buttons
2. Include quick presets (1,2,3)
3. Store times_target in habit record
4. Writes dialog spec to tests/specs/E2T3.json and appends to tests/commands.json
...
If this task adds a bot command, declare it in its OWN file `tests/commands/E2T3.json` (a JSON array of command strings, e.g. `["/start"]`). NEVER edit or append to a shared `tests/commands.json` — same conflict reason. The tests-gate globs, merges + de-duplicates all `tests/commands/*.json`.
