# E3T3 — Primary action buttons for habits

**Weight:** 0.0000 (share of project budget)
**Reward:** 0 STRKLY

Add habit action buttons:
1. '✓ Done'/'Done ✅' for daily/weekly
2. '+1' and 'Undo' for times/day
3. Button state updates after interaction
4. Writes dialog spec to tests/specs/E3T3.json and appends to tests/commands.json
...
If this task adds a bot command, declare it in its OWN file `tests/commands/E3T3.json` (a JSON array of command strings, e.g. `["/start"]`). NEVER edit or append to a shared `tests/commands.json` — same conflict reason. The tests-gate globs, merges + de-duplicates all `tests/commands/*.json`.
