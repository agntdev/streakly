# E2T1 — /add command for habit name input

**Weight:** 0.0000 (share of project budget)
**Reward:** 0 STRKLY

Implement /add command that:
1. Prompts for habit name via text input
2. Validates name length
3. Transitions to frequency selection (E2T2)
4. Writes dialog spec to tests/specs/E2T1.json and appends to tests/commands.json
...
If this task adds a bot command, declare it in its OWN file `tests/commands/E2T1.json` (a JSON array of command strings, e.g. `["/start"]`). NEVER edit or append to a shared `tests/commands.json` — same conflict reason. The tests-gate globs, merges + de-duplicates all `tests/commands/*.json`.
