...
Extending `buildBot()`, implement `/help` (lists the commands), a friendly fallback for unknown commands, and a global error boundary so an unhandled error replies gracefully instead of crashing the bot. Write the relevant dialog specs to your OWN `tests/specs/T03.json` (a BotSpec array) and declare `/help` in your OWN `tests/commands/T03.json` (a JSON string array) — never a shared file.
