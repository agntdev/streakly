# E4T1 — Edit habit details

**Weight:** 0.0000 (share of project budget)
**Reward:** 0 STRKLY

Implement habit editing:
1. Allow changing name, frequency, target, weekdays
2. Preserve optional reminder time field
3. Update habits list message
4. Writes dialog spec to tests/specs/E4T1.json and appends to tests/commands.json
...
If this task adds a bot command, declare it in its OWN file `tests/commands/E4T1.json` (a JSON array of command strings, e.g. `["/start"]`). NEVER edit or append to a shared `tests/commands.json` — same conflict reason. The tests-gate globs, merges + de-duplicates all `tests/commands/*.json`.
