# E6T1 — /stats command with streak analytics

**Weight:** 0.0000 (share of project budget)
**Reward:** 0 STRKLY

Display statistics:
1. Today's progress (done vs total)
2. Longest streak across all habits
3. Mini-progress lines per habit
4. Writes dialog spec to tests/specs/E6T1.json and appends to tests/commands.json
...
If this task adds a bot command, declare it in its OWN file `tests/commands/E6T1.json` (a JSON array of command strings, e.g. `["/start"]`). NEVER edit or append to a shared `tests/commands.json` — same conflict reason. The tests-gate globs, merges + de-duplicates all `tests/commands/*.json`.
