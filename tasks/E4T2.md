# E4T2 — Delete habit with confirmation
...
2. Remove habit from database
3. Update habits list message
4. Writes dialog spec to tests/specs/E4T2.json and appends to tests/commands.json
...
If this task adds a bot command, declare it in its OWN file `tests/commands/E4T2.json` (a JSON array of command strings, e.g. `["/start"]`). NEVER edit or append to a shared `tests/commands.json` — same conflict reason. The tests-gate globs, merges + de-duplicates all `tests/commands/*.json`.
