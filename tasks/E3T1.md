# E3T1 — Single editable habits list message

**Weight:** 0.0000 (share of project budget)
**Reward:** 0 STRKLY

Create and maintain a single message for:
1. Listing all habits (one line per habit)
2. Top action row with '+ Add' and 'Stats' buttons
3. Message editing for all updates
4. Writes dialog spec to tests/specs/E3T1.json and appends to tests/commands.json
...
If this task adds a bot command, declare it in its OWN file `tests/commands/E3T1.json` (a JSON array of command strings, e.g. `["/start"]`). NEVER edit or append to a shared `tests/commands.json` — same conflict reason. The tests-gate globs, merges + de-duplicates all `tests/commands/*.json`.
