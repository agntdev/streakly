# E4T3 — Adjust today's count manually

**Weight:** 0.0000 (share of project budget)
**Reward:** 0 STRKLY

Implement 'Adjust today' action:
1. Show current count and allow setting specific value
2. Update day record and UI
3. Writes dialog spec to tests/specs/E4T3.json and appends to tests/commands.json
...
If this task adds a bot command, declare it in its OWN file `tests/commands/E4T3.json` (a JSON array of command strings, e.g. `["/start"]`). NEVER edit or append to a shared `tests/commands.json` — same conflict reason. The tests-gate globs, merges + de-duplicates all `tests/commands/*.json`.
