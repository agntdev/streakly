# E1T1 — /start command with first-run setup

**Weight:** 0.0000 (share of project budget)
**Reward:** 0 STRKLY

Implement /start command that:
1. Detects first run and shows timezone selection (E1T2)
2. For non-first run, displays the main habits list message with 'Add' and 'Stats' buttons
3. Uses editMessageText for subsequent updates
4. Writes dialog spec to tests/specs/E1T1.json and appends to tests/commands.json
