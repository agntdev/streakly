# E3T4 — Details menu with ... button

**Weight:** 0.0000 (share of project budget)
**Reward:** 0 STRKLY

Implement habit details menu:
1. '...' button per habit
2. Menu with 'Edit', 'Delete', 'History' actions
3. Context-sensitive button states
4. Writes dialog spec to tests/specs/E3T4.json and appends to tests/commands.json
...
If this task adds or changes user-facing bot behavior, author its dialog tests as a `BotSpec` JSON array in its OWN file `tests/specs/E3T4.json`. NEVER edit or append to a shared `tests/specs.json` — concurrent feature PRs would conflict on it. The tests-gate globs and merges all `tests/specs/*.json`.

If this task adds a bot command, declare it in its OWN file `tests/commands/E3T4.json` (a JSON array of command strings, e.g. `["/start"]`). NEVER edit or append to a shared `tests/commands.json` — same conflict reason. The tests-gate globs, merges + de-duplicates all `tests/commands/*.json`.
